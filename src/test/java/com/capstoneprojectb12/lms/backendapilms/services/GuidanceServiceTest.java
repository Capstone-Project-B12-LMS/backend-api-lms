...
//		success
        when(this.guidanceRepository.findById(anyString())).thenReturn(Optional.of(guidance));
        ...
        assertNotNull(data);
        assertTrue(data.isPresent());
        assertInstanceOf(Guidance.class, data.get());
        assertTrue(api.isStatus());
        reset(this.guidanceRepository);

//		guidance not found
        when(this.guidanceRepository.findById(anyString())).thenReturn(Optional.empty());
        res = this.guidanceService.deleteById("id");
        api = getResponse(res);
        data = extract(guidance, api);
        assertEquals(HttpStatus.BAD_REQUEST, res.getStatusCode());
        assertNotNull(api.getErrors());
...
//		any exception
        when(this.guidanceRepository.findById(anyString())).thenThrow(AnyException.class);
        res = this.guidanceService.deleteById("id");
        api = getResponse(res);
        data = extract(guidance, api);
        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, res.getStatusCode());
