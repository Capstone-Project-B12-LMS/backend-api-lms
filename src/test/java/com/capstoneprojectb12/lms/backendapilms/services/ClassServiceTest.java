...
//		 success
        ...
        assertEquals(classEntity.getName(), data.get().getName());
        assertEquals(classEntity.getRoom(), data.get().getRoom());
...
//		 failed
    ...
    public void testUpdate() {
        ...
        assertEquals(classEntity.getName(), data.get().getName());
        assertEquals(classEntity.getRoom(), data.get().getRoom());
        assertEquals(classEntity.getStatus(), data.get().getStatus());
        assertEquals(classEntity.getReportUrl(), data.get().getReportUrl());
        reset(this.classRepository);
...
//		handle any exception
    ...
    public void testDeleteById() {
        ...
        var deletedData = extract(deleted(new Class()), api);

        assertEquals(HttpStatus.OK, result.getStatusCode());
        assertTrue(api.isStatus());
        assertNull(api.getErrors());
        assertNotNull(api.getData());
        assertTrue(api.getData() instanceof ResponseDelete);
        assert Objects.equals(classEntity.getName(), deletedData.get().getDeleted().getName());
        reset(this.classRepository);
        ...
        deletedData = extract(deleted(classEntity), api);
...
//		success
        when(this.classRepository.findById(anyString())).thenReturn(Optional.of(classEntity));
        when(this.classRepository.save(any(Class.class))).thenReturn(classEntity);
        var res = this.classService.updateStatus("id", ClassStatus.WILL_END);
        var api = getResponse(res);
        var data = extract(classEntity, api);
        assertEquals(HttpStatus.OK, res.getStatusCode());
        assertNull(api.getErrors());
...
//		class not found
        when(this.classRepository.findById(anyString())).thenReturn(Optional.empty());
        when(this.classRepository.save(any(Class.class))).thenReturn(classEntity);
        res = this.classService.updateStatus("id", ClassStatus.WILL_END);
        api = getResponse(res);
        data = extract(classEntity, api);
        assertEquals(HttpStatus.BAD_REQUEST, res.getStatusCode());
        assertNotNull(api.getErrors());
